...
ROCK_CYCLE_SIZE = ROCK_SHAPES.size.lcm($input.size)

# simulate one full cycle
ROCK_SHAPES.cycle.each_with_index do |rock_shape, rock_number|
  print_cave(occupied_positions, max_y(occupied_positions)) and break if rock_number == ROCK_CYCLE_SIZE
...
num_full_cycles = ROCK_COUNT / ROCK_CYCLE_SIZE
tower_height *= num_full_cycles

remaining_rocks = ROCK_COUNT % ROCK_CYCLE_SIZE

puts(ROCK_CYCLE_SIZE)
puts(tower_height)
puts(num_full_cycles)
puts(remaining_rocks)
